...
plot(b_scot,rug=FALSE,scheme=1,n=500,ylim=c(0,175),ylab="Daily infections/Deaths",
...
dat <- simple_model_dat(ed_BEL, "2020-03-10", 2)
...
b <- simple_model_b(ed_BEL, "2020-03-10", 2)



par(mar=c(5,5,1,1),mfrow=c(3,1))
plot(b,rug=FALSE,scheme=1,n=500,ylim=c(0,600),ylab="Belgium Daily infections/Deaths", xlab = "",select=1,cex.lab=1.2);
...
par(mar=c(5,5,1,1),mfrow=c(1,1))
plot(b_esp,rug=FALSE,scheme=1,n=500,ylim=c(0,2100),ylab="Daily infections/Deaths",
     xlab="Days since 31 Dec 2019",select=1,cex.lab=1.2);
abline(v=lock_esp);abline(0,0,lty=2)
with(dat_esp,points(tag,tote,pch=19,col=1,cex=.5))
lines(dat_esp$tag,fitted(b_esp),col="grey",lwd=2)
...
par(mar=c(5,5,1,1),mfrow=c(1,1))
plot(b_ita,rug=FALSE,scheme=1,n=500,ylim=c(0,1700),ylab="Daily infections/Deaths",
     xlab="Days since 31 Dec 2019",select=1,cex.lab=1.2);
abline(v=lock_ita);abline(0,0,lty=2)
with(dat_ita,points(tag,tote,pch=19,col=1,cex=.5))
lines(dat_ita$tag,fitted(b_ita),col="grey",lwd=2)
...
par(mar=c(5,5,1,1),mfrow=c(1,1))
plot(b_dnk,rug=FALSE,scheme=1,n=500,ylim=c(0,50),ylab="Daily infections/Deaths",
     xlab="Days since 31 Dec 2019",select=1,cex.lab=1.2);
abline(v=lock_dnk);abline(0,0,lty=2)
with(dat_dnk,points(tag,tote,pch=19,col=1,cex=.5))
lines(dat_dnk$tag,fitted(b_dnk),col="grey",lwd=2)
...
#lock_fra <- c(julian(as.Date("2020-3-17"),origin=as.Date("2019-12-31")),
              #julian(as.Date("2020-10-30"),origin=as.Date("2019-12-31")),
              #julian(as.Date("2021-04-03", origin = as.Date("2019-12-31"))))

#dat_fra <- simple_model_dat(ed_FRA, "2020-02-15", 1)
#b_fra <- simple_model_b(ed_FRA, "2020-02-15", 1)
